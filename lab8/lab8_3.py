...
def main():
        ...
        index = find_index(list, search_value)

        # Display the result
        if index != -1:
            print(find_index(list, search_value))
        else:
            print(-1)

def find_index(list, search_value):
    """
    Function to find the index of the search_value in the list.
    Returns -1 if the value is not found.
    """
    for i, num in enumerate(list): #enumerate ดึง index ของค่านั้น ๆ
        if num == search_value:
            return i
    return -1
